...
bool isBump( int diferencaZ ){
    ...
    return ( ( ( lista[elementosMedicao-2] - desvioPadrao ) < diferencaZ )
    && ( ( lista[elementosMedicao-2] + desvioPadrao ) > diferencaZ) ) ;
}
...
void loop() {
    ...
    if( isGravar( gps.getLatitude(), gps.getLongitude(), gps.getVelocidade() ) ){
        formato.setDados( acelerometro.getEixoX(), acelerometro.getEixoY(),
        acelerometro.getEixoZ(), gps.getLongitude(),
        gps.getLatitude(), gps.getVelocidade(), gps.getTime() );
